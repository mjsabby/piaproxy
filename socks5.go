...
// Logf is a logger function. It is compatible with log.Printf.
...
type Server struct {
	...
	// connsMu guards activeConns *and* the per-Conn meta fields below.
	// Counters (rxBytes/txBytes) are updated with atomic adds outside the
	// lock and read either with atomic loads (snapshot) or with the lock
	// held (anywhere we want a consistent group of fields).
	connsMu     sync.Mutex
	activeConns map[*Conn]struct{}
}

// ConnInfo is one entry in Server.Snapshot(): a single live SOCKS5
// client connection.  Bytes are counted from the proxy's perspective:
//
//	Tx = client → upstream (uploads)
//	Rx = upstream → client (downloads)
...
// Snapshot returns one ConnInfo per active client connection.
func (s *Server) Snapshot() []ConnInfo {
	s.connsMu.Lock()
	defer s.connsMu.Unlock()
	out := make([]ConnInfo, 0, len(s.activeConns))
...
type Conn struct {
	...
	// Liveness bookkeeping for Server.Snapshot().  peer/dest/method/
	// startMs are written once during the request handshake and then
	// only read; rxBytes/txBytes are updated with atomic adds for the
	// lifetime of the proxy stream.
	peer    string
	dest    string
	method  string
	startMs int64
...
// countWriter wraps an io.Writer and atomically adds every successful
// write count to *n.  Used to instrument the bidirectional io.Copy
// inside handleTCP without forking the SOCKS5 fast path.
...
func (c *Conn) handleTCP() error {
	c.dest = c.request.destination.hostPort()
	c.method = "tcp"
	...
	go func() {
		// upstream → client : the client's "download" side, counted as Rx.
		_, err := io.Copy(&countWriter{w: c.clientConn, n: &c.rxBytes}, srv)
		if err != nil {
	...
	go func() {
		// client → upstream : the client's "upload" side, counted as Tx.
		_, err := io.Copy(&countWriter{w: srv, n: &c.txBytes}, c.clientConn)
		if err != nil {
